namespace YAGO.World.Infrastructure.Datasets.Common
{
    public static class WikiArticleConstants
    {
        public const string LifeSolar = "life_solar";
        public const string LifeShareholders = "life_shareholders";
        public const string LifeQuatlas = "life_quatlas";
        public const string LifeHelium3 = "life_helium3";
        public const string LifeFirebird = "life_firebird";
        public const string LifeNanotubes = "life_nanotubes";
        public const string LifeSpaceTreaty = "life_spaceTreaty";
        public const string LifeSpaceElevator = "life_spaceElevator";
        public const string LifeKesslerCascade = "life_kesslerCascade";
        public const string LifeGoldenRing = "life_goldenRing";

        public const string StationDawn = "station_dawn";
        public const string StationResolute = "station_resolute";

        public const string GameplayYago = "gameplay_yago";
    }
}