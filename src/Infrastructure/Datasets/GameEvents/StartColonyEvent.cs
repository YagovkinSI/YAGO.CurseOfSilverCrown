    ...
    public static class StartColonyEvent
    {
        ...
        private static Slide GetSlide5()
        {
            return new Slide(
                ...
                text: new string[] {
                    "«Станция и стартовый капитал — это кредит, который Консорциум выдал колонии под проценты. Это не ваш личный долг, а долг самой колонии — что-то вроде государственного долга стран на Земле.",
                    "Проценты по кредиту — одна из основных статей расходов колонии в первое время.»"},
                parameterChanges: [],
                buttons: [
                    SlideButton.GetCloseNewsButton(Id, "Подписать контракт"),
                    SlideButton.GetButtonToSlide($"{Id}_3", "Почему я?"),
